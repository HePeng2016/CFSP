...
int
tre_regncomp(regex_t *preg, const char *regex, size_t n,void * buf,int cflags)
{
  int ret;
#if TRE_WCHAR
  tre_char_t *wregex;
  ...
  wregex[wlen] = L'\0';
  ret = tre_compile(preg, wregex, wlen,buf,cflags);
  xfree(wregex);
#else /* !TRE_WCHAR */
  ret = tre_compile(preg, (const tre_char_t *)regex, n,buf, cflags);
#endif /* !TRE_WCHAR */
...
int
tre_regcomp(regex_t *preg, const char *regex,void * buffer,int cflags)
{
  return tre_regncomp(preg, regex, regex ? strlen(regex) : 0, buffer,cflags);
}


#ifdef TRE_WCHAR
int
tre_regwncomp(regex_t *preg, const wchar_t *regex, size_t n, void * buffer,int cflags)
{
  return tre_compile(preg, regex, n, buffer,cflags);
}

int
tre_regwcomp(regex_t *preg, const wchar_t *regex, void * buffer,int cflags)
{
  return tre_compile(preg, regex, regex ? wcslen(regex) : 0,buffer, cflags);
}
#endif /* TRE_WCHAR */
