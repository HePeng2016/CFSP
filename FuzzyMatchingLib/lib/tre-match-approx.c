...
reg_errcode_t
tre_tnfa_run_approx( tre_tnfa_t *tnfa, const void *string, int len,
		    ...
		    int eflags, int *match_end_ofs)
{
  ...
  {
	{
     ...
     // assert(tnfa->reachTraceArray[0] == tnfa->alloc); 
        tnfa->alloc = tnfa->reachTraceArray+len*sizeof(reachTrace*);  
        tnfa->reachTraceArray[0] = tnfa->alloc;
        reachTracedebug = tnfa->reachTraceArray[0];
  
      for( i=0;i<tnfa->num_states;i++)
	  {
		reachTracedebug[i].parentID =-1;
		reachTracedebug[i].pos =0;
	  }
	}
  }
    ...
    tnfa->reachTraceArray[1] = tnfa->alloc + tnfa->num_states;
    reachTracedebug = tnfa->reachTraceArray[1];
     for( i=0;i<tnfa->num_states;i++)
     {
		reachTracedebug[i].parentID =-1;
		reachTracedebug[i].pos =0;
  ...
  while (/*CONSTCOND*/1)
      ...
      for (id = 0; id < tnfa->num_states; id++)
	{
	  ...
	  reach_next[id].pos = pos;
      	...
      	if(reachTracedebug[id].parentID<0||(tnfa->Cost[id] > cost0))
        {
		  reachTracedebug[id].parentID =id;
	      reachTracedebug[id].pos = pos;
	      tnfa->Cost[id] = cost0;
        }
	...
	while (deque_end != deque_start)
	  {
		...
		if(reachTracedebug[dest_id].parentID<0||(tnfa->Cost[dest_id] >= cost0))
        {
		  reachTracedebug[dest_id].parentID =reachTracedebug[id].parentID;
		  reachTracedebug[dest_id].pos = pos;
		  tnfa->Cost[dest_id] = cost0;
        }
		...
		*deque_end = &reach_next[dest_id];
	    ...
	    if (deque_start >= (ringbuffer + 512))
	      deque_start = ringbuffer;
            ...
            for (id = 0; id < tnfa->num_states; id++)
            {
              if( reachTracedebug[id].parentID != -1)
              {
                 if( reach_next[id].params.max_cost < tnfa->Cost[id] )
                {
                    reachTracedebug[id].parentID = -1;
                }
              }
            }
      ...
      else
	{
      ...
      /* Swap `reach' and `reach_next'. */
      {
		...
		reach = reach_next;
		reach_next = tmp;
		tnfa->reachTraceArray[pos] = tnfa->alloc + pos*tnfa->num_states;
		reachTracedebug = tnfa->reachTraceArray[pos];
        for( i=0;i<tnfa->num_states;i++)
        {
            reachTracedebug[i].parentID =-1;
            reachTracedebug[i].pos =pos;
        }
      ...
      for (id = 0; id < tnfa->num_states; id++)
	{
	  ...
	  for (trans = reach[id].state; trans->state; trans++)
		  ...
		  || trans->code_max < (tre_cint_t)prev_c)
		{
          ...
          reachTracedebug[dest_id].parentID =id;
		  reachTracedebug[dest_id].pos = pos;
		  tnfa->Cost[dest_id] = cost;
	  ...
	  match_costs[TRE_M_COST]));
  {
	...
	int rootlength = 0;
        ...
        for(j=len;j>0;j--)
	  {
	      reachTracedebug = tnfa->reachTraceArray[j];

	      for(i=0;i<tnfa->num_states;i++)
	    {
            if((reach[i].state == tnfa->final||reach_next[i].state ==tnfa->final)&&(reachTracedebug[i].parentID>0))
		   {
			  rootstate[rootlength]=i;
			  rootpos[rootlength]=j;
			  rootlength++;
		   }
         }
         if(rootlength !=0)
   ...
   {
	   int index = rootstate[i];
	   j=rootpos[i];

	   index = reachTracedebug[index].parentID;
	   j--;
       if(index < 0)
       {
         continue;
      ...
      for(i=0;i<tnfa->num_states;i++)
        match->matchresult[i] = -1;

	   for(;j>=0;j--)
	   {
		  reachTracedebug = tnfa->reachTraceArray[j];
		//  printf("%d-",reachTracedebug[index].pos);
		//  printf("%d	",index);
		  match->matchresult[index] = reachTracedebug[index].pos;
                  index = reachTracedebug[index].parentID;
